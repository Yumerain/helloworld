...
public class ClientProcessor extends Thread {
	...
	public void run() {
		...
		try {
			...
			logger.error("处理来自[" + socket.getInetAddress().getHostAddress() + ":" + socket.getPort() + "]的客户端socket出错.", e);
		} finally {
			try {
				socket.close();
			} catch (IOException e) {
	...
	private String readHeadLine(InputStream is) throws IOException, HttpProtocolException {
		...
		while(true){
			...
			if(buff.length()>HEADLINE_MAX){
				logger.error("请求头部单行数据超过设定最大值：" + buff.toString());
				throw headExcpt;
			}
	...
	private void readAllHeaders(InputStream is) throws IOException, HttpProtocolException {
		...
		while(count>0){
			...
			if(splitIndex == -1){
				logger.error("不符合HTTP协议的头部：" + line);
				throw headExcpt;
			}
